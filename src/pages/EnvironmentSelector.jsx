...
const EnvironmentSelector = ({ 
  ...
  const handleAddEnvironment = () => {
    if (newEnvName.trim() && !environments.includes(newEnvName)) {
      onAddEnvironment(newEnvName);
      setNewEnvName('');
                ...
                <div className="flex items-center gap-1 ml-2">
                  {env !== 'Default' && onCopyEnvironment && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        const cloneName = prompt(`Clone "${env}" as:`, `${env}-clone`);
                        if (cloneName && cloneName.trim()) {
                          onCopyEnvironment(env, cloneName);
                        }
                      }}
